...
export function formatMlaReference(md: ReferenceMetadata) {
  const authors = formatAuthors(md);
  const title = `"${md.title}"`;
  const container =
    md.sourceType === 'journal'
      ? `${md.journal || ''} ${[md.volume, md.issue].filter(Boolean).join('.')}${md.pages ? `: ${md.pages}` : ''}`
      : md.publisher;
  return joinNonEmpty([authors, title, container, md.year, md.doi || md.url]);
}

export function formatMlaInText(md: ReferenceMetadata, opts: { first: boolean; page?: string }) {
